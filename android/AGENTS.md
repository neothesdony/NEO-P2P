...
- **Payment window:** the buyer can mark the fiat payment as sent (`markPaid` → `PAID`, `paid_at`). The seller then has 2 h (`PAYMENT_WINDOW_MS`) to release or dispute; if the window expires, `expireStaleEscrows()` auto-`DISPUTED` the escrow — never silently auto-refunded, because the buyer may have actually paid.

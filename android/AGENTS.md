...
- **P2P Networking (Phase 4 — RNS is the ONLY transport):**
  ...
  - `data/p2p/RnsSession.kt` — Pure-JVM RNS + LXMF core (client-only, TCP client to the VPS transport node)
  - `data/p2p/RnsTransport.kt` — Android wrapper (P2PTransport impl)
...
- RNS transport node address in `NeoP2PConfig.RNS_TRANSPORT_NODE_HOST/PORT` (VPS rnsd-kt, port 42000)

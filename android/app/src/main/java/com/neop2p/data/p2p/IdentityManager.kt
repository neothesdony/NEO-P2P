 ...
 *
 * - Ed25519 keypair stored in Android KeyStore (hardware-backed where available)
 * - BIP-39 seed phrase for backup/restore
 * - Derived identities: libp2p PeerID, Nostr npub, Lightning node ID
 */
@Singleton
class IdentityManager @Inject constructor(
    ...
    fun getOrCreateIdentity(): Identity {
        ...
        if (keyStore.containsAlias(KEYSTORE_ALIAS)) {
            ...
            val identity = buildIdentity(publicKey, privateKey, seedPhrase = null)
    ...
    /**
     * Generates a new Ed25519 keypair and stores in Android KeyStore.
     */
    private fun generateNewIdentity(): Identity {
        val keyPairGenerator = KeyPairGenerator.getInstance(
            KeyProperties.KEY_ALGORITHM_ED_25519,
            KEYSTORE_PROVIDER
        )
        ...
        // Generate BIP-39 seed phrase from key material
        val seedPhrase = generateSeedPhrase()

        val identity = buildIdentity(keyPair.public, keyPair.private, seedPhrase)
        cachedIdentity = identity
        return identity
    }
    ...
    fun restoreFromSeedPhrase(seedPhrase: List<String>): Identity {
        // Derive key from seed + store in KeyStore
        // For v1: generate fresh and save seed reference
        // Full derivation from mnemonic requires BIP-32 — coming in Phase 2
        return getOrCreateIdentity().copy(seedPhrase = seedPhrase)
    }

    /**
     * Builds the Identity object from keys.
     */
    private fun buildIdentity(
        ...
        return Identity(
            publicKey = publicKey,
            privateKey = privateKey,
            peerId = "12D3KooW" + bytesToBase58(sha256.take(14).toByteArray()),
            nostrPubkeyHex = bytesToHex(sha256),
            seedPhrase = seedPhrase ?: emptyList()
        )
    }

    private fun generateSeedPhrase(): List<String> {
        // Simplified for v1 — 12 words from BIP-39 wordlist
        // Full BIP-39 implementation using novacrypto/BIP39 in Phase 2
        val secureRandom = SecureRandom()
        val entropy = ByteArray(16)  // 128 bits = 12 words
        secureRandom.nextBytes(entropy)
        return entropy.map { BIP39_WORDS[it.toInt() and 0xFF].lowercase() }
    }

    companion object {
        // BIP-39 English wordlist (first 256 for simplified generation)
        val BIP39_WORDS = listOf(
            ...
            "daughter", "dawn", "day", "deal", "debut", "decade", "decide", "decline",
            ...
            "evoke", "evolve", "exact", "example", "exceed", "exchange", "excite", "exclude",
            ...
            "foreign", "forest", "forget", "fork", "fortune", "forum", "forward", "fossil",
            ...
            "rigid", "ring", "riot", "rip", "ripe", "rise", "risk", "rival",
            ...
            "state", "stay", "steak", "steel", "step", "stereo", "stick", "still",
            ...
            "suggest", "suit", "sun", "sunny", "sunset", "super", "supply", "support",
    ...
    private fun bytesToBase58(bytes: ByteArray): String {
        // Simplified base58 — for v1 demo purposes
        // Full base58 check encoding (Base58Check) coming in Phase 2

...
| A5 | App upgrade with old state files | COVERED | Room v25 migrations (7→25 documented in AGENTS.md); `fallbackToDestructiveMigrationOnDowngrade` (2026-09-02); legacy identity migration (IdentityManager.kt:397-416). |
...
| C3 | Cancel locally; remote stale cache | COVERED | DeletedOfferStore tombstone + ingest skip (OfferRouter.kt:283-287); terminal statuses never resurrect (OfferRouter.kt:304-328). **2026-09-06:** a terminal tombstone DELETES the row on an OBSERVER (neither creator nor matched peer) so a finished trade disappears from the feed entirely (`OfferFeedGate.tombstoneDeletesRow`); party rows stay (marked COMPLETED) — the buyer's escrow detail reads fiat + bank details from the offer row. |
| C4 | Order TTL expire | COVERED | expires_at (DB v21), claim gate past expiry, stale badge (HomeScreen.kt:814-823). **2026-09-05:** expired OPEN/PAUSED offers are auto-deleted by the 60s sweep (`sweepExpiredOffers` — Room row dropped, digest untracked, tombstoned). |
...
| ID | Scenario | Status | Handler / evidence |
|---|---|---|---|
| D1 | Happy path accept | COVERED | applyOfferStatus (OfferRouter.kt:104-235) + OfferClaimGateTest. **2026-09-06:** an OPEN effective status clears the match (`clearsMatch` → `matched_peer_id` + `locked_at` nulled) so a declined/re-activated offer's former taker can re-accept. |
...
| D4 | Negotiate then ghost | COVERED | Funding timeout auto-cancel (EscrowService.kt:481-543); payment window auto-dispute (569-601). **2026-09-05:** a MATCHED offer whose escrow is never created is auto-CANCELLED 24h after the match (`locked_at`, Room v25, `sweepStaleMatchedOffers` — creator-gated, synced to the former matched peer over LXMF). |
| D5 | Terms change after lock | COVERED | Locked offers immutable (no-downgrade; edit gated to OPEN/PAUSED; editing MATCHED/ESCROWED blocked 2026-09-06). |
...
| E13 | Payout to fee wallet / own multisig | **COVERED 2026-09-07** | `PayoutAddressGate.isForbidden` (pure) rejects the fee wallet and the escrow's own funding address at accept AND at build; `resolveBuyerPayoutAddress` resolves escrow row → offer row and NEVER falls back to the multisig (that fallback paid the buyer's sats back into the escrow). PayoutAddressGateTest + EscrowPayoutAddressResolverTest. |
| E14 | Refund fee mismatch (estimate vs broadcast) | **COVERED 2026-09-06** | `refundNetworkFeeSats` (full tx vsize + 250-sat floor) shared by `buildRefundTx` and `getRefundEstimate` — the displayed refund amount equals the broadcast refund. EscrowFeeMathTest. |
...
| F2 | Cancel after lock before funding | COVERED | Auto-cancel FUNDING (EscrowService.kt:481-543) + offer marked CANCELLED + sync. **2026-09-05:** stale MATCHED (escrow never created) auto-cancels after 24h (`sweepStaleMatchedOffers`). |
...
| F5 | Dispute evidence bundles | COVERED | LXMF dispute/evidence/resolution; arbitrator_disputes Room v23 (buyer/seller peerIds for resolution delivery); EscrowArbitrationResolutionTest; DisputeRedeliveryGateTest. **2026-09-05:** FUNDING is not disputable (`canDisputeFromStatus` — deposit not confirmed, nothing to arbitrate; the 45-min window auto-cancels). |
...
- COVERED: 53 · UNKNOWN: 0 · FAILING: 0 · N/A: 6 · ACCEPTED: 6

...
*Filed: 2026-09-01. State at time of writing: fork on v1.0.26, 323 tests passing, RNS/LXMF is the ONLY transport since v1.0.22.*
...
## 3. How This Fork Actually Uses It (verified from code + CHANGELOG 1.0.22–1.0.26)

- **Phase 4: RNS/LXMF is the ONLY transport.** `LibP2PManager`, `P2PTransportManager`, `NostrClient`, `WebRTCManager`, `HybridP2PTransport` were deleted (v1.0.22). `RnsSession`/`RnsTransport` connect as TCP clients to the VPS transport node (`NeoP2PConfig.RNS_TRANSPORT_NODE_HOST/PORT`).
...
- **Infrastructure** (`infrastructure/`): `rns-transport` (**official Python rnsd** since 2026-09-01 — replaced the rnsd-kt fork; the two fork fixes that were needed in rnsd-kt, spawned-client registration + receiving-interface wiring, are native Python behavior; `config` file bind-mounted `:ro` — rnsd loads exactly `File(dir, "config")`, no extension) + `lxmf-propagation` (LXMF Propagation Node, `lxmd --config <dir> --rnsconfig <dir> -p`) on Oracle ARM64. Node config REQUIRES `announce_rate_target=1`, `announce_rate_grace=20`, `announce_rate_penalty=0` on every interface (the Python default `3600` blocks the app's destinations for an hour). TCP keepalive on the client interface, 20s delivery re-announce to keep NAT/firewall links alive.
- **Test harness**: 62 test files including `RnsSessionTest`, `RnsLoadTest`, `RnsSoakTest`, `RnsThreePeerTest`, `RnsTwoProcessIntegrationTest`, `RnsLatencyTest`, `RnsFaultInjectionTest` (fault proxy, flap server, offer flood server).
...
- **Frozen upstream**: RNS 1.5.1 is mature but unmaintained (maintainer stepped back; mirror-only public repo). The JVM fork (rns-core/lxmf-core, JDK 21 bytecode) is owned by this project — fixes land here (client registration, LINKREQUEST wiring, config loading) and must keep landing here. The transport node runs the **official Python rnsd** (reference implementation) so the two fork fixes are no longer needed server-side.
...
- **Rate limits**: the fork's path admission caps announces per destination per 30s; the 2.5s paced loop respects it with ~25% headroom (verified zero drops at production cadence by RnsLoadTest). The Python transport node's own `announce_rate_target` must be set to 1 (grace 20, penalty 0) or it blocks app destinations for an hour — documented as REQUIRED in `infrastructure/AGENTS.md`.

...
- **Payment window (2026-08-25):** the buyer can mark the fiat payment as sent (`markPaid` → `PAID`). The seller then has **2 h** to release or dispute; if the window expires the escrow auto-transitions to `DISPUTED` — never silently auto-refunded, because the buyer may have actually paid and the arbitrator decides with evidence.

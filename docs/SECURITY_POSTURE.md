...
- **Fee model (0.3%, seller-only):** the seller deposits `crypto + 0.3% fee + network fee`; the buyer pays no fee and receives the full crypto amount; the 0.3% goes to the fee wallet. **Money is integer-only (G.M.01, 2026-08-29):** `feeSats = (sats * FEE_NUM) / FEE_DEN` (`3/1000`) and `fiatAmount = (btcSats * priceIdr) / 100_000_000` — no `Double` round-trip on money; a non-whole typed price is rejected (`parseIdrToLong`) so it can never be misread as a 10x integer.

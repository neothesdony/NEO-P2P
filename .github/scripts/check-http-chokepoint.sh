...
# the explorer allow-list / fail-closed behaviour.
set -euo pipefail

repo_root="$(cd "$(dirname "$0")/../.." && pwd)"
src_dir="$repo_root/android/app/src/main/java"

# Files allowed to mention HTTP client construction.
allowlist_re='(di/AppModule\.kt|data/escrow/ChainMonitor\.kt):[0-9]+:'

violations="$(grep -RInE 'OkHttpClient|HttpClient\(|HttpURLConnection|java\.net\.URL' "$src_dir" --include='*.kt' 2>/dev/null \
